...
class INET_API IPv4RouteRule : public cPolymorphic
{
  ...
  protected:
    ...
    int sPort;  ///
    int dPort;  ///
    bool active; // this means a rule is known to the node, even if it might be not active (in case a routing rule has been added and then deleted)
  ...
  public:
    ...
    bool isEnforced(){return enforced;}
    void enforce(){enforced=true;}
    void unenforce(){enforced=false;}
